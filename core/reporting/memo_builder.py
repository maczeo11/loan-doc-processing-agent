...
def _verdict_badge(verdict: str) -> str:
    ...
    return _VERDICT_BADGES.get(v, "❓ UNKNOWN")
...
def build_appraisal_summary(
    ...
    raw_name = _get_val(app_obj, "full_name")
    app_name = raw_name if raw_name and raw_name.strip().upper() != "UNKNOWN" else "UNKNOWN"
    ...
    applicant_summary = {
        ...
        "name_citation": _format_evidence_citation(_get_val(app_obj, "source_name")),
        "pan_citation": _format_evidence_citation(_get_val(app_obj, "source_pan")),
    }
...
def build_appraisal_memo(
    ...
    applicant_header_name = raw_name if raw_name and raw_name.strip().upper() != "UNKNOWN" else "Unknown Applicant"
    lines.append(f"# Credit Appraisal Memo (CAM) — {app_id}")
    lines.append(f"### Credit Appraisal Memo — {app_id}")
    lines.append(f"**Applicant Name:** {applicant_header_name}")
    lines.append("")
    ...
    applicant_display_name = raw_name if raw_name and raw_name.strip().upper() != "UNKNOWN" else "UNKNOWN"
    ...
    lines.append(f"- **Applicant Full Name:** {applicant_display_name}")
    ...
    if not findings:
        ...
        for idx, finding in enumerate(findings, 1):
            ...
            lines.append(f"- **{rule_name}** ({rule_id}) [{badge}]: {reason}")
    ...
    if chunk_ids:
        lines.append(f"Referenced guidelines: {', '.join(f'[{c_id}]' for c_id in chunk_ids)}")
        for c_id in chunk_ids:
            lines.append(f"- Referenced Policy Passage: [{c_id}]")
    else:
        lines.append("Referenced guidelines: None")
