# 🌐 FinScan AI — Complete End-to-End System Workflow & Architecture Blueprint
## 🏛️ Comprehensive Technical Specification: All Tools, Databases, Pipelines & Integrations

---

## 🧭 Executive Overview & The Prime Invariant

FinScan AI is an enterprise-grade retail loan document verification and credit appraisal system designed for bank underwriters. It eliminates manual document sorting and calculation errors while strictly honoring the project's **Prime Invariant**:
...
1. **Zero Hallucinated Decisions:** No total income, net salary, debt-to-income (DTI) ratio, or pass/flag verdict originates from an LLM. Pure deterministic Python computes arithmetic; AI only narrates and explains.
2. **Mandatory Provenance:** Every extracted fact requires an `EvidenceRef` containing document ID, page number, and bounding-box coordinates. If unreadable, it is marked `UNKNOWN`.
3. **No Autonomous Lending:** The system halts at an explicit LangGraph `interrupt()` checkpoint. Final disposition requires a human underwriter clearing a 3-tier Dual-Sign friction gate.
4. **Zero-Node Production:** React 18 frontend compiles to static assets served same-origin by FastAPI. Zero Node.js processes run on the production server.

---

## 🏗️ 1. Master System Architecture Diagram (Mermaid)

The following diagram illustrates every physical layer, process boundary, database, cache, and port adapter in the system:

```mermaid
flowchart TB
    subgraph ClientLayer ["1. Client Layer (Browser SPA)"]
        UI["React 18 + Vite + TypeScript"]
        Canvas["pdf.js Canvas Viewer"]
        Overlay["BoundingBoxOverlay Engine"]
        FrictionGate["3-Tier Dual-Sign Friction Gate"]
        UI --- Canvas
        Canvas --- Overlay
        UI --- FrictionGate
    end

    subgraph EdgeReverseProxy ["2. Reverse Proxy & Security Edge"]
        Caddy["Caddy Server (HTTPS / Let's Encrypt)"]
        StaticServe["Same-Origin Static Assets (/dist)"]
        Caddy --- StaticServe
    end

    subgraph APILayer ["3. API Backend Layer (FastAPI)"]
        FastAPI["FastAPI Application (Python 3.11)"]
        AuthMiddleware["JWT / Google OAuth Context"]
        RateLimiter["Redis Token Bucket Rate Limiter"]
        OpenAPISchema["Frozen OpenAPI v3 Schema"]
        
        FastAPI --- AuthMiddleware
        FastAPI --- RateLimiter
        FastAPI --- OpenAPISchema
    end

    subgraph CachingLayer ["4. Transient Cache & State (Redis 7)"]
        RedisBucket["Token Buckets (30 uploads/min, 30 polls/min)"]
        RedisQaCache["Policy Q&A Answer Cache (TTL: 24h)"]
        RedisSession["Active User Session Store"]
    end

    subgraph PersistenceLayer ["5. Authoritative Persistence (PostgreSQL 16)"]
        PgBouncer["PgBouncer Connection Pooler (Transaction Mode)"]
        subgraph PGDB ["PostgreSQL Relational DB"]
            T_Apps[("applications")]
            T_Docs[("documents")]
            T_Outbox[("outbox_jobs (SKIP LOCKED)")]
            T_Audit[("audit_trail (Immutable)")]
            T_Checkpoints[("langgraph_checkpoints (PostgresSaver)")]
        end
        PgBouncer --- PGDB
    end

    subgraph StorageLayer ["6. Object Storage (StoragePort)"]
        S3_Local["StoragePort Adapter"]
        LocalStorage["Local FileSystem (Local Dev)"]
        AWSS3["AWS S3 Bucket (AES-256 SSE, Presigned URLs)"]
        S3_Local --> LocalStorage
        S3_Local --> AWSS3
    end

    subgraph QueueLayer ["7. Asynchronous Message Broker (QueuePort)"]
        QueueAdapter["QueuePort Adapter"]
        PGQueue["PostgreSQL SKIP LOCKED Queue (Local)"]
        SQSQueue["AWS SQS Queue + DLQ (Ceiling: 3 Retries)"]
        QueueAdapter --> PGQueue
        QueueAdapter --> SQSQueue
    end

    subgraph WorkerLayer ["8. Distributed Async Worker Subsystem"]
        WorkerConsumer["Worker Process (consumer.py)"]
        Heartbeat["LeaseHeartbeat Thread (Every 10s -> Extends 30s)"]
        AckLast["Acknowledge-Last Guard (Commit DB -> ACK)"]
        WorkerConsumer --- Heartbeat
        WorkerConsumer --- AckLast
    end

    subgraph PipelineLayer ["9. LangGraph StateGraph (8-Station Workflow)"]
        S1["Station 1: triage_node"]
        S2["Station 2: ocr_and_classify_node"]
        S3["Station 3: extract_facts_node"]
        S4["Station 4: evaluate_rules_node"]
        S5["Station 5: retrieve_policy_node"]
        S6["Station 6: synthesize_summary_node"]
        S7["Station 7: validate_grounding_node"]
        S8["Station 8: human_review_node (INTERRUPT)"]
        
        S1 --> S2 --> S3 --> S4 --> S5 --> S6 --> S7 --> S8
    end

    subgraph PerceptionTools ["Perception & ML Tools (Stations 2 & 3)"]
        PyMuPDF["PyMuPDF (fitz) Native Text Extraction"]
        PaddleOCR["PaddleOCR CPU Engine (Scanned Fallback)"]
        Textract["AWS Textract (Capped < 100 pages, Off by Default)"]
        BaselineML["TF-IDF + Logistic Regression (1.2ms, 15MB RAM)"]
        ChallengerML["DistilBERT Transformer (Lazy Escalation 0.15-0.40)"]
    end

    subgraph RAGAndLLM ["RAG & Model Serving (Stations 5 & 6)"]
        BM25["BM25 Lexical Search (Exact Alphanumeric Codes)"]
        BGE["BGE-Small-EN-v1.5 Dense Embeddings (384-dim)"]
        FAISS["FAISS IndexFlatIP (In-Memory Flat Index)"]
        RRF["Reciprocal Rank Fusion (k=60)"]
        LLM["LLMPort (OpenCode Zen / Qwen3-4B Local GGUF)"]
    end

    subgraph ReportingExport ["10. Archival & Reporting (Station 8+)"]
        ReportLab["ReportLab Platypus PDF Generator"]
        JSONExport["Structured JSON Dossier Archiver"]
    end

    %% Wiring Connections
    UI ==>|HTTPS / REST| Caddy
    Caddy ==> FastAPI
    FastAPI <-->|Rate Limit & Cache| RedisBucket
    FastAPI <-->|Rate Limit & Cache| RedisQaCache
    FastAPI ==>|Queries & Atomic Outbox| PgBouncer
    FastAPI ==>|Store Encrypted Dossiers| S3_Local

    T_Outbox -.->|Poll SKIP LOCKED| QueueAdapter
    QueueAdapter ==>|Lease Job Delivery| WorkerConsumer
    WorkerConsumer ==>|Instantiates & Resumes| PipelineLayer

    S2 --- PerceptionTools
    S5 --- RAGAndLLM
    S6 --- LLM
    S7 ---|Prompt Injection Defense| S7
    S8 <==|Resume with Underwriter Decision| FastAPI
    S8 ==> ReportingExport
```

---

## 🔄 2. Complete End-to-End Sequence Workflow (From Upload to Funding)

![FinScan AI Complete End-to-End Sequence Workflow Infographic](C:\Users\balaj\.gemini\antigravity-ide\brain\20eb99d4-3e1c-49a6-b87c-e99c2cfb90d5\finscan_workflow_diagram_1789316033473.jpg)

Here is the exact, step-by-step chronology of a loan application traveling through the entire FinScan AI infrastructure:

```mermaid
sequenceDiagram
    autonumber
    actor Underwriter as Human Underwriter (Priya)
    participant UI as React 18 SPA (Vite + pdf.js)
    participant API as FastAPI Backend (Balaji)
    participant Redis as Redis 7 (Cache & Rate Limiter)
    participant DB as PostgreSQL 16 (PgBouncer)
    participant Storage as StoragePort (Local / AWS S3)
    participant Queue as QueuePort (PG / AWS SQS)
    participant Worker as Async Worker (Bhanu Teja)
    participant Graph as LangGraph StateGraph
    participant ML as ML Classifier & OCR (Jeevan / Karthik)
    participant Rules as Deterministic Rules Engine (Sravanthi)
    participant RAG as Hybrid RAG & Grounding (Sai Mokshith)

    %% PHASE 1: INGESTION
    Note over Underwriter, Storage: PHASE 1: INGESTION & RATE LIMITING
    Underwriter->>UI: Uploads 15-page mixed dossier (PDFs)
    UI->>API: POST /api/v1/applications (Create Dossier)
    API->>Redis: Check Token Bucket (Limit: 30 req/min)
    Redis-->>API: Bucket OK (Tokens remaining)
    API->>Storage: Store PDF (SHA-256 deduplicated, AES-256 SSE)
    Storage-->>API: Storage Key (dossiers/APP-25195/doc_raw.pdf)
    API->>DB: INSERT INTO applications (Status: UPLOADED)
    DB-->>API: Application Created (APP-25195)
    API-->>UI: 201 Created (APP-25195)

    %% PHASE 2: ATOMIC DISPATCH
    Note over API, Worker: PHASE 2: TRANSACTIONAL OUTBOX & QUEUEING
    Underwriter->>UI: Clicks "Run Verification Pipeline"
    UI->>API: POST /api/v1/applications/APP-25195/process
    rect rgb(240, 248, 255)
        Note over API, DB: Atomic DB Transaction (Single Commit)
        API->>DB: UPDATE applications SET status = 'QUEUED'
        API->>DB: INSERT INTO outbox_jobs (job_id, payload, status='PENDING')
        DB-->>API: Commit Successful
    end
    API-->>UI: 202 Accepted (job_id: JOB-UUID, status: QUEUED)
    
    API->>DB: SELECT * FROM outbox_jobs WHERE status='PENDING' FOR UPDATE SKIP LOCKED
    DB-->>API: Claimed 1 Pending Job
    API->>Queue: publish(JobRef) -> SQS / PG Queue
    API->>DB: UPDATE outbox_jobs SET status='DISPATCHED'

    %% PHASE 3: WORKER CONSUMPTION & LEASE
    Note over Queue, Worker: PHASE 3: ASYNC WORKER LEASE & HEARTBEAT
    Queue->>Worker: receive() -> Delivery(Handle, JobRef)
    Worker->>Worker: Start LeaseHeartbeat Thread (extend_lease every 10s)
    Worker->>DB: UPDATE applications SET status='PROCESSING'
    Worker->>Graph: Initialize LangGraph StateGraph (App State)

    %% PHASE 4: LANGGRAPH EXECUTION
    Note over Graph, RAG: PHASE 4: THE 8-STATION LANGGRAPH PIPELINE
    Graph->>Graph: Station 1: triage_node (Validate manifest)
    
    Graph->>ML: Station 2: ocr_and_classify_node
    ML->>ML: PyMuPDF Native Text Parser (or PaddleOCR fallback)
    ML->>ML: FeatureUnion (Word+Char n-grams) + Logistic Regression
    ML->>ML: Multi-page Consensus Voting (aggregate_document_predictions)
    ML-->>Graph: Classified: Payslips, Bank Statement, Tax Return, ID Card

    Graph->>ML: Station 3: extract_facts_node
    ML->>ML: Extract Net Salary, Bank Credits, GTI, PAN with BoundingBoxes
    ML-->>Graph: Domain Fact Models with EvidenceRefs

    Graph->>Rules: Station 4: evaluate_rules_node (HUMAN-ONLY ZONE)
    Rules->>Rules: RULE-COMP-01: Check presence of 5 mandatory documents
    Rules->>Rules: RULE-INC-01: Reconcile Payslip Net vs Bank Credits (Tol: 5%)
    Rules->>Rules: RULE-TAX-01: Compare ITR GTI vs Annualized Payslip Gross
    Rules->>Rules: RULE-ID-01 & RULE-ID-02: Fuzzy string matching on PAN / Name
    Rules-->>Graph: Typed Findings (PASS, FLAG, UNKNOWN)

    Graph->>RAG: Station 5: retrieve_policy_node
    RAG->>RAG: BM25 Lexical Search + BGE Dense Embeddings
    RAG->>RAG: Reciprocal Rank Fusion (RRF k=60) on Policy Corpus
    RAG-->>Graph: Retrieved Top-k Policy Rules

    Graph->>RAG: Station 6: synthesize_summary_node
    RAG->>RAG: Assemble Credit Appraisal Memo Markdown Narrative (OpenCode/Qwen)
    RAG-->>Graph: Draft CAM Narrative with bracketed citations

    Graph->>RAG: Station 7: validate_grounding_node
    RAG->>RAG: Regex Adversarial Pattern Scanner (Prompt Injection Defense)
    RAG->>RAG: Grounding Validation Gate (Verify all citations exist in chunks)
    RAG-->>Graph: Grounding Verified -> Advance to READY_FOR_REVIEW

    %% PHASE 5: THE INTERRUPT CHECKPOINT
    Note over Graph, Underwriter: PHASE 5: THE INTERRUPT CHECKPOINT & HUMAN REVIEW
    Graph->>Graph: interrupt_before: human_review_node (HALT PIPELINE)
    Worker->>DB: PostgresSaver commits checkpoint (State: READY_FOR_REVIEW)
    Worker->>Queue: ack(Handle) (Acknowledge-Last: Queue message deleted)
    
    UI->>API: Poll GET /applications/APP-25195 (Every 2s)
    API-->>UI: Status: READY_FOR_REVIEW + Findings + EvidenceRefs
    UI->>UI: Center Pane renders PDF with dynamic BoundingBox highlights!
    
    %% OPTIONAL INTERACTIVE Q&A
    opt Underwriter asks a policy question
        Underwriter->>UI: "Explain salary mismatch flag"
        UI->>API: POST /applications/APP-25195/questions
        API->>Redis: Check Q&A Cache for question hash
        alt Cache Miss
            API->>RAG: Hybrid Retrieve & Synthesize Answer
            RAG-->>API: Answer + [POL-SAL-01] citation
            API->>Redis: SET answer with 24h TTL
        else Cache Hit
            Redis-->>API: Return cached verified response
        end
        API-->>UI: Verified Answer + ShieldCheck Badge
    end

    %% PHASE 6: DUAL-SIGN SIGN-OFF
    Note over Underwriter, DB: PHASE 6: 3-TIER DUAL-SIGN FRICTION GATE & SEALING
    Underwriter->>UI: Presses 'R' (Initiate Rejection)
    UI->>UI: Opens ReviewActionModal (Tier 1: REJECT selected)
    Underwriter->>UI: Types Rationale (Tier 2: "Income inflated by 19.6%")
    Underwriter->>UI: Types Challenge (Tier 3: "APP-25195")
    UI->>UI: "Confirm Rejection" button unlocks!
    Underwriter->>UI: Clicks Confirm

    UI->>API: POST /applications/APP-25195/review {decision: REJECTED, notes: "..."}
    API->>Graph: Resume Checkpoint via graph.invoke()
    Graph->>Graph: Station 8: human_review_node (Applies Underwriter Verdict)
    Graph-->>API: State: REVIEWED (Sealed)

    API->>DB: UPDATE applications SET status='REVIEWED'
    API->>DB: INSERT INTO audit_trail (actor, timestamp, decision, rationale)
    API-->>UI: 200 OK (Status: REVIEWED)

    %% PHASE 7: EXPORT
    Note over Underwriter, Storage: PHASE 7: OFFICIAL DISPOSITION EXPORT
    Underwriter->>UI: Clicks "Download Audit Report (PDF)"
    UI->>API: GET /applications/APP-25195/export?format=pdf
    API->>API: ReportLab Platypus renders signed CAM with Official Disposition Seal
    API-->>UI: Streams publication-quality PDF receipt
    UI-->>Underwriter: Download complete (Permanent Legal Audit Record)
```

---

## 🚦 3. The 8 LangGraph Stations & Their Concrete Responsibilities

The execution pipeline follows a deterministic state transition machine:
...
[evaluate_rules_node]      (Deterministic Rules: Completeness, Salary, Tax, Identity)
         │
         ▼
[retrieve_policy_node]     (Hybrid RAG: BM25 + BGE Dense on Policy Manuals)
         │
         ▼
[synthesize_summary_node]  (Assembles CAM Narrative via Structured Prompts)
         │
         ▼
[validate_grounding_node]  (Citation Gate: Transitions state to READY_FOR_REVIEW)
         │
         ▼
[interrupt_before: human_review_node] <─── 🛑 UNCONDITIONAL HALT FOR HUMAN REVIEW
         │ (Resumed via API with Reviewer Decision: APPROVED | REJECTED | NEEDS_INFO)
         ▼
[human_review_node] ──────► Transitions to REVIEWED or NEEDS_INFORMATION ──► [END]
```

### Station Summary Table:
| Station | Function Name | Input Data | Core Technology & Algorithms | Output Artifacts |
| :--- | :--- | :--- | :--- | :--- |
| **Station 1** | `triage_node` | Raw file manifest | Pydantic v2 validation | `PROCESSING` state |
| **Station 2** | `ocr_and_classify_node` | PDF bytes / paths | PyMuPDF, PaddleOCR CPU, TF-IDF + Logistic Regression, Hybrid Cascade | `classified_types`, page texts |
| **Station 3** | `extract_facts_node` | Page text + bounding boxes | Regex financial patterns, coordinate scaling | Typed Facts (`PayslipFacts`, `BankStatementFacts`, etc.) with `EvidenceRef`s |
| **Station 4** | `evaluate_rules_node` | Extracted Facts | Pure deterministic Python math ($\le 0.05$ tolerance) | Typed `Finding`s (`RULE-COMP-01`, `RULE-INC-01`, `RULE-TAX-01`, `RULE-ID-01`, `RULE-ID-02`) |
| **Station 5** | `retrieve_policy_node` | Rule findings & queries | BM25 Lexical + BGE-Small-EN dense vectors, FAISS Flat index, RRF ($k=60$) | Top-k Policy Chunks with Chunk IDs |
| **Station 6** | `synthesize_summary_node` | Policy chunks + Findings | OpenCode Zen / Qwen3-4B Local GGUF, contextual delimiters | Markdown Credit Appraisal Memo |
| **Station 7** | `validate_grounding_node` | Draft CAM narrative | Regex injection filter, Citation Grounding Gate | State advances to `READY_FOR_REVIEW` |
| **Station 8** | `human_review_node` | Underwriter submission | 3-Tier Dual-Sign Friction Gate, PostgreSQL checkpointer resume | `REVIEWED` or `NEEDS_INFORMATION`, immutable audit log |

---

## 🗄️ 4. Data Layer Architecture: PostgreSQL, Redis & S3

FinScan AI partitions data storage according to durability, access speed, and compliance needs:

```mermaid
graph LR
    subgraph S3Storage ["Object Storage (StoragePort)"]
        RawPDF["Raw Uploaded PDFs (AES-256 SSE)"]
        SignedCAM["Generated CAM PDFs & JSON Exports"]
    end

    subgraph PostgresDB ["PostgreSQL 16 (Authoritative Persistence)"]
        AppsTable["applications (Dossier lifecycle & metadata)"]
        DocsTable["documents (Per-file status, SHA-256, paths)"]
        OutboxTable["outbox_jobs (SKIP LOCKED transactional queue)"]
        AuditTable["audit_trail (Append-only reviewer sign-offs)"]
        CheckpointsTable["langgraph_checkpoints (PostgresSaver thread state)"]
    end

    subgraph RedisCache ["Redis 7 (Transient In-Memory Cache)"]
        RateBuckets["Token Buckets (Upload: 30/m, Poll: 30/m)"]
        PolicyCache["Policy Q&A Answer Cache (TTL: 24h)"]
        ActiveSessions["Reviewer Active JWT Session Store"]
    end

    RawPDF --- DocsTable
    AppsTable --- OutboxTable
    AppsTable --- AuditTable
    AppsTable --- CheckpointsTable
```

### Table Schema & Guarantees:
1. **`outbox_jobs`:** Implements the transactional outbox pattern. Uses `SELECT ... FOR UPDATE SKIP LOCKED` so concurrent worker processes never pick up the same job.
2. **`langgraph_checkpoints`:** Stores the binary serialized graph state at every station via `PostgresSaver`. If a server crashes during extraction, the pipeline resumes from the exact station without re-running prior steps!
3. **`audit_trail`:** Append-only ledger recording every review override: `actor`, `timestamp`, `from_status`, `to_status`, `rationale`. Rows cannot be updated or deleted.
4. **Redis Token Buckets:** Enforces rate-limiting quotas (30 uploads/min, 30 polls/min) to prevent cloud spend overages.

---

## 🛡️ 5. Security, Guardrails & Banned Technologies Matrix

| Threat / Risk | Defense Mechanism | Responsible File & Station |
| :--- | :--- | :--- |
| **Prompt Injection in PDFs** | Document text is quoted and isolated via contextual delimiters; regex scanner detects overrides; Grounding Gate deletes ungrounded claims. | `core/rag/grounding.py` (Station 7) |
| **Accidental Loan Approvals** | 3-Tier Dual-Sign Friction Gate: Intent Selection $\rightarrow$ Mandatory Rationale ($\ge 5$ chars) $\rightarrow$ Dossier ID Challenge (`APP-XXXXX`). | `ReviewActionModal.tsx` (Station 8) |
| **Cloud Spend Overruns** | Hard budget ceiling ($25/week). AWS Textract hard-capped $<100$ pages (off by default); Redis rate limits; local Qwen/CPU fallbacks. | `apps/api/config.py`, `router.py` |
| **PII Data Exposure** | PAN, Aadhaar, and Bank Account numbers are masked to last 4 characters (`XXXXXX1234`) in UI and CAM exports. | `apps/ui/src/utils/pii.ts` |
| **Multi-Page Data Leakage** | Machine learning train/dev/test splits are strictly grouped by `applicant_id` to prevent layout leakage. | `ml/classifier/baseline_tfidf.py` |
| **Autonomous AI Lending** | LangGraph unconditionally halts at `interrupt_before: human_review_node`. AI cannot issue final verdicts. | `core/graph/workflow.py` |

### Explicitly Banned Technologies:
* ❌ **No Next.js / Server-Side Rendering (SSR):** Replaced by pure static Vite build served by FastAPI (Zero-Node production).
* ❌ **No Tabular XGBoost / LightGBM for Credit Decisions:** Legal regulatory trap under RBI Fair Lending rules.
* ❌ **No Kafka / Celery:** Replaced by lightweight PostgreSQL `SKIP LOCKED` and AWS SQS + DLQ.
* ❌ **No Redux / MobX:** Replaced by lightweight React Context and ephemeral local state.
* ❌ **No GPU Cloud Instances:** Baseline models and FAISS run 100% on standard ARM/x86 CPU cores.

---

## 🎯 6. Master Viva Summary: How All 8 Members Connect

```
Member 1 (Manjunath)  ──► Defines Contracts (EvidenceRef, MoneyFact, Finding, LoanApplicationState)
                                │
Member 2 (Balaji)     ──► Builds FastAPI REST Endpoints, PostgreSQL Outbox, PgBouncer & Cloud Host
                                │
Member 3 (Bhanu Teja) ──► Orchestrates LangGraph StateGraph, Async Worker Loop & Lease Heartbeats
                                │
Member 5 (Jeevan)     ──► Runs OCR Routing (PyMuPDF -> PaddleOCR) & Bounding-Box Fact Extraction
                                │
Member 6 (Karthik)    ──► Classifies Pages via TF-IDF Baseline (1.2ms) & Hybrid Cascade DistilBERT
                                │
Member 4 (Sravanthi)  ──► Executes Deterministic Financial Rules (HUMAN-ONLY ZONE) & Generates Data
                                │
Member 8 (Sai Mokshith)──► Performs Hybrid RAG (BM25 + BGE), RRF Fusion & Grounding Validation Gate
                                │
Member 7 (Akshaya)    ──► Renders Three-Pane Cockpit, pdf.js Canvas & Enforces 3-Tier Dual-Sign Gate
```

> **The System In a Single Sentence:**  
> *"FinScan AI takes a raw multi-page loan dossier, routes it through CPU-native OCR and machine learning classifiers in under 2 milliseconds, extracts financial facts with exact bounding boxes, audits salary and tax claims using deterministic Python math, synthesizes a policy-grounded appraisal memo, and halts for a human underwriter to sign off through a 3-tier safety friction gate."*
